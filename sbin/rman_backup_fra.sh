#!/usr/bin/env bash
# ------------------------------------------------------------------------------------------------------------
# ----  Script Name: rman_backup_fra.sh
# ----  Versioninformation
...
# ----  Short Description: RMANSCRIPT: Backup Oracle Database FRA - Fast Recovery Area - FRA is always FULL!! 
# ------------------------------------------------------------------------------------------------------------
# ---- BEFORE:
...
if [ $VARCOUNT -ne 2 ];then
 echo "Please provide ORACLE_SID as first and BACKUP_DIR=fra as second input parameter. VARCOUNT=${VARCOUNT}";
 exit 1;
fi
...
# RMAN Call
echo "rman cmdfile=${RMAN_SCRIPT_BASE_PATH}/rman_backup_fra.rcv $ORACLE_SID \"$BACKUP_DIR\" "
rman cmdfile=${RMAN_SCRIPT_BASE_PATH}/rman_backup_fra.rcv $ORACLE_SID \"$BACKUP_DIR\"
