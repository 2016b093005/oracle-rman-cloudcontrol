#!/usr/bin/env bash
# ------------------------------------------------------------------------------------------------------------
# ----  Script Name: rman_backup_database_rfi_rollforward.sh
# ----  Versioninformation
...
if [ $VARCOUNT -ne 4 ];then
 echo "Please provide ORACLE_SID, BACKUP_DIR=rfi, BACKUP_TAG=RFI, BACKUP_UTIME_DAYS=3. VARCOUNT=${VARCOUNT}";
 exit 1;
fi
...
# RMAN Call
echo "rman cmdfile=${RMAN_SCRIPT_BASE_PATH}/rman_backup_database_rfi_rollforward.rcv $ORACLE_SID \"$BACKUP_DIR\" \"$BACKUP_TAG\" \"$BACKUP_UTIME_DAYS\" "
rman cmdfile=${RMAN_SCRIPT_BASE_PATH}/rman_backup_database_rfi_rollforward.rcv $ORACLE_SID \"$BACKUP_DIR\" \"$BACKUP_TAG\" \"$BACKUP_UTIME_DAYS\"
