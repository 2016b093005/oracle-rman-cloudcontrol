#!/usr/bin/env bash
# ------------------------------------------------------------------------------------------------------------
# ----  Script Name: rman_backup_database_rfi_l1.sh 
# ----  Versioninformation
...
# ----  Short Description: RMANSCRIPT: Backup the Database INCREMENTAL L1 - with Rolling Forward Images
# ------------------------------------------------------------------------------------------------------------
# ---- BEFORE:
...
# TASKNAME/CONDITION: os command to call
 ...
 # BACKUP_DATABASE_RFI_L1/ALWAYS: /usr/local/sbin/rman_backup_database_rfi_l1.sh %SID% rfi RFI 1 ""
# ------------------------------------------------------------------------------------------------------------
...
export BACKUP_INC_LEVEL=$4
export BACKUP_INC_CUM=$5
export VARCOUNT=$#
...
# Prechecks
if [ $VARCOUNT -ne 5 ];then
 echo "Please provide ORACLE_SID, BACKUP_DIR=rfi, BACKUP_TAG=RFI, BACKUP_INC_LEVEL=1, BACKUP_INC_CUM="". VARCOUNT=${VARCOUNT}";
 exit 1;
fi
...
# RMAN Call
echo "rman cmdfile=${RMAN_SCRIPT_BASE_PATH}/rman_backup_database_rfi_l1.rcv $ORACLE_SID \"$BACKUP_DIR\" \"$BACKUP_TAG\" \"$BACKUP_INC_LEVEL\" \"$BACKUP_INC_CUM\" "
rman cmdfile=${RMAN_SCRIPT_BASE_PATH}/rman_backup_database_rfi_l1.rcv $ORACLE_SID \"$BACKUP_DIR\" \"$BACKUP_TAG\" \"$BACKUP_INC_LEVEL\" \"$BACKUP_INC_CUM\"
