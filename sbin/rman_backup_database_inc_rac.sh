#!/usr/bin/env bash                                                                                                                                                                          
# ------------------------------------------------------------------------------------------------------------                                                                               
# ----  Script Name: rman_backup_database_inc_rac.sh
# ----  Versioninformation                                                                                                                                                                   
...
# ---- ORATAB:          no hacks inside this file (be aware of automatic changes when using RAC) 
# ----                  and only one database instance inside and startflag=Y                                                                                      
...
# TASKNAME/CONDITION: os command to call
 ...
 # BACKUP_RACDB_COMP_L0: /usr/local/sbin/rman_backup_database_inc_rac.sh %SID% database 0 " " AS COMPRESSED BACKUPSET BACKUP_INC_L0 %DBName%
...
# Prechecks
if [ $VARCOUNT -ne 9 ];then
 echo "Please provide at least ORACLE_SID,BACKUP_DIR, BACKUP_LEVEL, BACKUP_CUMULATIV, BACKUP_AS, BACKUP_COMP, BACKUP_TYPE, BACKUP_TAG and ORACLE_DB is input vars. VARCOUNT=${VARCOUNT}";
 echo "compressed - level 0: scriptname.sh RACSID database 0 \" \" AS COMPRESSED BACKUPSET DATABASE_INC RACDBNAME"
 echo "uncompressed - level 0: scriptname.sh RACSID database 0 \" \" \" \" \" \" \" \" DATABASE_INC RACDBNAME"
 echo "compressed - level 1: scriptname.sh RACSID database 1 \" \" AS COMPRESSED BACKUPSET DATABASE_INC RACDBNAME"
 echo "uncompressed - level 1: scriptname.sh RACSID database 1 \" \" \" \" \" \" \" \" DATABASE_INC RACDBNAME"
echo "compressed - level 1c: scriptname.sh RACSID database 1 cumulative AS COMPRESSED BACKUPSET DATABASE_INC RACDBNAME"
 echo "uncompressed - level 1c: scriptname.sh RACSID database 1 cumulative \" \" \" \" \" \" DATABASE_INC RACDBNAME"
 exit 1;
fi
...
BACKUP_FULL_DIR="${BACKUP_BASE}/${ORACLE_DB}/${BACKUP_DIR}"
...
# RMAN Call
echo "rman cmdfile=/usr/local/etc/rman/rman_backup_database_inc.rcv $ORACLE_DB \"$BACKUP_DIR\" \"$BACKUP_LEVEL\" \"$BACKUP_CUMULATIV\" \"$BACKUP_AS\" \"$BACKUP_COMP\" \"$BACKUP_TYPE\" \"$BACKUP_TAG\" "
rman cmdfile=/usr/local/etc/rman/rman_backup_database_inc.rcv $ORACLE_DB \"$BACKUP_DIR\" \"$BACKUP_LEVEL\" \"$BACKUP_CUMULATIV\" \"$BACKUP_AS\" \"$BACKUP_COMP\" \"$BACKUP_TYPE\" \"$BACKUP_TAG\"
