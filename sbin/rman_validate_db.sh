#!/usr/bin/env bash
# ------------------------------------------------------------------------------------------------------------
# ----  Script Name: rman_validate_db.sh
# ----  Versioninformation
# ----  Modification History:
# ----  Date,           Author,         Desc.,
# ----  2018-05-06,     J.Halwachs,     github creation 
...
if [ $VARCOUNT -ne 1 ];then
 echo "Please provide ORACLE_SID as input parameters. VARCOUNT=${VARCOUNT}";
 echo "example: scriptname.sh PROD1P5";
 exit 1;
fi
...
# RMAN Call
echo "rman cmdfile=/usr/local/etc/rman/rman_validate_db.rcv $ORACLE_DB"
rman cmdfile=/usr/local/etc/rman/rman_validate_db.rcv $ORACLE_DB
